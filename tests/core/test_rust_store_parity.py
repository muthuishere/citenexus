...
def core() -> ctypes.CDLL:
    ...
    lib.citenexus_store_open.restype = ctypes.c_void_p
    lib.citenexus_store_open.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
    lib.citenexus_store_upsert.restype = ctypes.c_void_p
    lib.citenexus_store_upsert.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
    lib.citenexus_store_search.restype = ctypes.c_void_p
    lib.citenexus_store_search.argtypes = [
        ctypes.c_void_p,
        ctypes.c_char_p,
        ctypes.c_size_t,
    ]
    lib.citenexus_store_scan.restype = ctypes.c_void_p
    lib.citenexus_store_scan.argtypes = [ctypes.c_void_p, ctypes.c_int64]
    lib.citenexus_store_drop.restype = ctypes.c_void_p
    lib.citenexus_store_drop.argtypes = [ctypes.c_void_p]
    lib.citenexus_store_close.restype = None
    lib.citenexus_store_close.argtypes = [ctypes.c_void_p]
...
def rust_written_store(core: ctypes.CDLL, tmp_path: Path) -> tuple[ctypes.CDLL, int, str]:
    ...
    handle = core.citenexus_store_open(uri.encode(), b"{}")
    assert handle, "citenexus_store_open returned null"
    ok = take_json(core, core.citenexus_store_upsert(handle, json.dumps(ROWS).encode()))
    assert ok == {"ok": True}
...
def test_python_scans_what_rust_wrote(
    ...
    try:
        ...
        core.citenexus_store_close(handle)
...
def test_python_searches_what_rust_wrote(
    ...
    try:
        ...
        rust_hits = take_json(
            core,
            core.citenexus_store_search(handle, json.dumps([0.0, 0.9, 0.1, 0.0]).encode(), 2),
        )
...
def test_rust_upsert_is_idempotent_when_python_reads(
    ...
    try:
        ...
        ok = take_json(core, core.citenexus_store_upsert(handle, json.dumps(updated).encode()))
        assert ok == {"ok": True}
...
def test_rust_drop_leaves_python_an_empty_leaf(
    ...
    core, handle, uri = rust_written_store
    try:
        ok = take_json(core, core.citenexus_store_drop(handle))
        assert ok == {"ok": True}
...
def test_rust_scans_what_python_wrote(core: ctypes.CDLL, tmp_path: Path) -> None:
    ...
    assert handle, "citenexus_store_open returned null"
    try:
        rust_rows = take_json(core, core.citenexus_store_scan(handle, -1))
