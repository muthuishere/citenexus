# vision-orchestration Specification

## Purpose

The two-phase, host-fulfilled contract for describing figures with a vision LLM
(ADR-0005, §9): the core **emits** model-ready requests, the **host fulfills**
them with its own transport, and the core **assembles** the citable figure
Evidence Units — so the polyglot core never opens a socket, never holds the API
key, and every deterministic decision stays byte-identical across ports.
...
Evidence Units SHALL be byte-identical across the Python, Go, and JS ports. The
emitted `payload`'s `image_url` SHALL declare the image's true media type, sniffed
from the magic bytes (png/jpeg/gif/webp, else png) rather than hardcoded. Only the
host-side fulfillment (the raw model call) may differ per language. These SHALL be
pinned as conformance fixtures.
