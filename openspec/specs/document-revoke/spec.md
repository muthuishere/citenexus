...
Retracting evidence is as first-class as adding it. `delete(document_id)` (alias
`revoke`) is the exact inverse of `ingest`: it removes one document and every
artifact derived from it so the document is neither retrievable nor citable, while
every other document stays intact and answerable. This supports corrections,
retractions, and right-to-be-forgotten erasure without rebuilding a corpus.
