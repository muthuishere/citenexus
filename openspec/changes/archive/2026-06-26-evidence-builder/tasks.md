...
- [x] 1.2 Confirm red: `uv run pytest tests/evidence -q` fails to import `trustrag.evidence.builder`

## 2. Implementation (green)

- [x] 2.1 `src/trustrag/evidence/builder.py`: closed `_KIND_TO_TYPE` table + `build_evidence_units(doc, *, partition, language, acl=None)` mapping each non-empty block to one `EvidenceUnit` in order
...
- [x] 3.2 `uv run ruff check src/trustrag/evidence tests/evidence` clean
- [x] 3.3 `uv run mypy src/trustrag/evidence tests/evidence` clean (strict)
