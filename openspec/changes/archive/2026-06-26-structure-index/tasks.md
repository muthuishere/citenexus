...
- [x] 1.2 Confirm red: `uv run pytest tests/evidence -q` fails to import `trustrag.evidence.structure`

## 2. Implementation (green)

- [x] 2.1 `src/trustrag/evidence/structure.py`: frozen `StructureNode` (node_id, parent_id, label, kind, eu_ref) + frozen `StructureIndex` (document_id, structure_type, nodes)
