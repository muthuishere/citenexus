...
- [x] 1.1 Add tree-sitter + grammar crates (Python + Go at minimum) to
      `rust/Cargo.toml`; confirm MIT licensing before pinning.
- [x] 1.2 Implement `rust/src/extract/code.rs`: one verbatim `code` block per
...
- [x] 1.3 Wire the `code` source type into `citenexus_extract` dispatch (Rust) and
      recognise code extensions (`.py`, `.go`); unknown extensions still fall back
      to plain.
- [x] 1.4 Add the Python **reference** implementation (`extract/code.py`) — used
      only as the parity oracle, not the shipped path.
- [x] 1.5 Red→green parity test `tests/core/test_rust_code_parity.py`: core output
...
- [x] 1.6 Unsupported-language source falls back to plain without raising.

## 2. `rag.code.ingest_from(folder | git)` intake verb (per-language facade)

- [x] 2.1 Add the `rag.code` namespace + `ingest_from(source)` verb in Python
...
- [x] 2.2 Fail-loud precondition: raise immediately if the instance has no
      `graph`/`community` signal — name the missing signal, ingest nothing. Test it.
- [x] 2.3 Red→green: folder ingest produces symbol EUs skipping vendored dirs; a
      git URL is cloned and ingested; missing-graph-signal raises.
- [x] 2.4 SDK wiring: Go/JS route each code file through `extract_ffi.go` / koffi to
...
- [x] 5.1 Fold the spike's `StructuralDistiller` into example/spike code (injected
...
- [x] 5.2 End-to-end (Python): ingest a small code corpus, inject the distiller,
...
- [x] 5.3 Green: `cargo test` (core + parity), Python `task lint`/`typecheck`/`test`,
      Go `go test ./...`, JS suite.
- [x] 5.4 Update `docs/SPEC-v6.md` / `docs/SPEC-PORTS-v1.md` / ADR-0006 to record:
