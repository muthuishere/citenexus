...
## 5. npm package + GitHub-Releases installer (red → green)

- [ ] 5.1 Failing test: the downloader verifies SHA256 and REFUSES a mismatched binary
- [ ] 5.2 Failing test: launcher lazy-downloads on first run when postinstall was skipped; cache-hit path re-execs without network; `CITENEXUS_BINARY` override bypasses download
- [ ] 5.3 Implement `@muthuishere/citenexus`: `bin/citenexus.mjs` launcher + `scripts/postinstall.mjs` (best-effort, non-fatal); cache `~/.cache/citenexus/<version>/<platform>`; `CITENEXUS_DOWNLOAD_BASE` + `HTTPS_PROXY`

## 6. CI release matrix

- [ ] 6.1 GH Actions workflow (SHA-pinned actions) builds `citenexus-<os>-<arch>` for darwin arm64/x64, linux x64/arm64, win x64 (static-linked) and publishes a Release with `SHA256SUMS`
- [ ] 6.2 npm publish job (`needs: build`) tags version == release tag

## 7. Docs

- [ ] 7.1 A `cli` docs page: install (`npm i -g @muthuishere/citenexus`), `init`, dual config, `${ENV}` secrets, the command table, `api` vs `skill` mode
- [ ] 7.2 Note the follow-on roadmap (okf, dashboard, skill mode, parity) so the page doesn't overclaim
