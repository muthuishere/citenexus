...
That is not the same as the problem going away. It **moves the whole audit into
your code**: whatever assembles that slice is now the thing an auditor has to
trust, and it is the thing that must be able to prove which documents it put in.
If you need the diagnostic itself — orphans, missing, drifted, and the
append-only audit line — the index has to be Python's.
</TabItem>
...
The audit obligation does not disappear, it relocates — the code that builds the
corpus array becomes the thing an auditor checks, and it must be able to say which
documents it included and why. For the orphan / missing / drifted report and its
append-only log, use the Python index.
</TabItem>
</Tabs>
...
<TabItem label="Go">
Nothing in this port writes the stream, because nothing in it reconciles. The
