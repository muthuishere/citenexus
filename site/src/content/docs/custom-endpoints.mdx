...
the model seam is a [published contract](/citenexus/providers/), and everything
on this page — `base_url`, `headers`, `transport` — belongs to the shipped HTTP
clients rather than to the contract. A provider that never opens a socket ignores
all of it.) Auth is a **header template**: you write `"Bearer ${OPENAI_API_KEY}"`, and the `${ENV_VAR}` is
...
Writing a model that is not an HTTP endpoint at all — in-process, a mock, an
adapter for a third library? None of this page applies:
