...
CiteNexus carries the partition hierarchy + an opaque ``acl`` but enforces
...
into a :class:`~citenexus.domain.partition.PartitionPath` prefix; ``prefilter``
gates candidate partitions (and, optionally, objects' opaque acl) against it.
"""

from citenexus.access.prefilter import (
    allowed_partition,
    apply_acl_predicate,
    filter_partitions,
)
from citenexus.access.scope import resolve_scope
