...
| 3 | Lists (bullet/numbered) | ✅ | ✅ `EUType.paragraph` (no dedicated list type) | ✅ | **2026-07-09: GREEN, corrected.** `extract/html.py:HtmlExtractor.extract` now also selects `li` (`_li_own_text` excludes any nested `<ul>/<ol>` subtree so a nested item isn't double-captured) — was genuinely dropped before, confirmed by direct extractor test. Re-verifying the doc's other two claims against the LIVE extractors found them **stale**: `md.py`'s tight-list items wrap their content in a `paragraph_open`/`inline`/`paragraph_close` triplet with `hidden=True` — the existing loop's `elif tok.type == "paragraph_open"` branch doesn't check `hidden`, so it already captured list items as paragraph blocks (confirmed by direct extractor test, pre-existing behavior, not a change made here). `docx.py` likewise already treated a `"List Bullet"`-styled paragraph as `BlockKind.paragraph` (`_heading_level` only recognizes `Heading*`) — also pre-existing, already reaching the store. No dedicated list `EUType` exists (numbering/nesting structure is lost across all three; only the text survives) — out of scope for "reach the evidence store". Proof: `tests/ingest/test_list_citation_e2e.py` — HTML, Markdown, and DOCX, each end to end through the public `CiteNexus` client, a question only a list item answers, cited. |
...
| 4b′ | Tables — DOCX/PPTX/HTML | ✅ | ✅ `EUType.table` | ✅ | **2026-07-09: GREEN.** `extract/docx.py:DocxExtractor.extract` now also walks `document.tables` (table-cell text is a separate part of the doc tree from `document.paragraphs`, so no duplication risk). `extract/pptx.py:PptxExtractor.extract` now checks `shape.has_table` per shape. `extract/html.py:HtmlExtractor.extract` now finds every `<table>` FIRST, renders its rows, then `.decompose()`s it before the heading/paragraph walk (so a `<p>` nested in a cell can't double-cite). All three render each data row `"col: value"` — same convention as `extract/csv.py`/PDF (row 4b) — → `BlockKind.table` → `EUType.table`. Proof, each through the public `CiteNexus` client: `tests/ingest/test_docx_pptx_table_citation_e2e.py` (DOCX + PPTX), `tests/ingest/test_html_table_citation_e2e.py` (HTML) — numeric question, cited passage is the table row. |
...
| 6a | Captions | ✅ | ✅ `EUType.paragraph` (no dedicated caption type) | ✅ | **2026-07-09: GREEN.** `extract/html.py:HtmlExtractor.extract` now captures `<figcaption>` (a figure's REAL caption — distinct from `VisionRecord.short_caption` in `vision/describe.py`, which is the vision *model's own generated* caption, an unrelated concept) and `<table><caption>`, both pulled out and decomposed before the main walk (same pattern as tables/lists/code). `extract/docx.py` needed no change: a paragraph styled `"Caption"` (Word's built-in style) was already just an unstyled `BlockKind.paragraph` to `_heading_level` — verified live before claiming it, same pattern as row 3's Markdown/DOCX correction. Proof: `tests/ingest/test_caption_citation_e2e.py` — HTML `<figcaption>`, HTML `<table><caption>`, and DOCX `"Caption"` style, each end to end through the public `CiteNexus` client. |
| 6b | Footnotes | ❌ | ❌ | ❌ | **Still open — genuinely out of reach with today's dependencies, not silently skipped.** `python-docx` 1.2.0 has no footnote-part API (only `footer_part`, an unrelated page-footer concept — confirmed by inspecting `Document().part`'s attributes directly). `markdown-it-py`'s footnote syntax (`[^1]`) needs the `mdit-py-plugins` package, not currently a project dependency (confirmed: `ModuleNotFoundError`). `pdfplumber` has no structural footnote/endnote marker — a PDF footnote is visually smaller text near the page bottom, not a distinguishable object in the extraction API. Would need either a new dependency (Markdown) or bespoke heuristics (PDF) — a real scoping decision, not a quick patch alongside captions. |
| 7 | Code blocks | ✅ | ✅ `EUType.code_block` | ✅ | **2026-07-09: GREEN.** `extract/md.py` now handles the `fence` (` ```lang `) and `code_block` (4-space indented) token types — each is a single token (not an open/close pair), literal source in `.content`, never inline-tokenized like paragraph/heading text. `extract/html.py` now finds every `<pre>` FIRST (preserving internal whitespace/newlines — code is whitespace-significant, unlike prose — no `strip=True`), then decomposes it before the heading/paragraph/list walk. Both map to `BlockKind.code` → `EUType.code_block` (mapping already existed, `evidence/builder.py:_KIND_TO_TYPE`). Proof: `tests/ingest/test_code_block_citation_e2e.py` — Markdown fence + HTML `<pre>/<code>`, each through the public `CiteNexus` client, a question only the code's literal content answers, cited. |
...
2. ~~**Tables work for CSV only.**~~ **RESOLVED 2026-07-09** — CSV (4a), PDF
   (4b), and DOCX/PPTX/HTML (4b′) all now extract real table rows to a
   citable `EUType.table`. Every content-source format now has table support.
3. ~~**Lists ... lost, not flattened, in HTML/Markdown.**~~ **RESOLVED
   2026-07-09** (row 3) — HTML `<li>` was genuinely fixed; Markdown/DOCX
   turned out to already reach the store (doc was stale on those two).
   ~~**Code blocks are lost, not flattened.**~~ **RESOLVED 2026-07-09**
   (row 7) — Markdown fences + HTML `<pre>/<code>` now reach a citable
   `EUType.code_block`.
4. ~~**Footnotes / captions.**~~ **Captions RESOLVED 2026-07-09** (row 6a) —
   HTML `<figcaption>`/`<table><caption>`, DOCX `"Caption"` style. **Footnotes
   remain open** (row 6b) — genuinely blocked on missing dependency/API
   support today, not a quick fix; see row 6b for exactly what's missing.
5. **No type-awareness at citation time.** Even where a typed EU exists
...
6. **No document metadata anywhere** — no title/author/date filtering is
   possible; only full-text/structure signals exist. Next up.
