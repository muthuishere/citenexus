...
``TRUSTRAG_`` prefix with ``__`` as the nesting separator, e.g.
``TRUSTRAG_TRUST__DEFAULT_MODE=strict`` sets ``trust.default_mode``.
"""
...
def _env_to_nested(env: Mapping[str, str], prefix: str) -> dict[str, Any]:
    """Turn ``TRUSTRAG_A__B=v`` style vars into ``{'a': {'b': 'v'}}``."""
    result: dict[str, Any] = {}
...
def from_config(
    ...
    *,
    ...
    """Build a :class:`TrustRAGConfig` from a source + overrides + environment.
    ...
    return TrustRAGConfig.model_validate(data)
