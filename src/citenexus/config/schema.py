...
pydantic sub-model, composed into the top-level :class:`CiteNexusConfig`. Defaults
...
class CiteNexusConfig(BaseModel):
    ...
    points at a ``citenexus.validate.yaml`` allow-list.
    """
    ...
    # Path to an optional ``citenexus.validate.yaml`` allow-list (the client
